...
from bot.helper.mirror_utils.download_utils.direct_link_generator import gdtot, AppDrive, HubDrive, SharerPw, sirigan
...
def cloneNode(update, context):
    ...
    if "appdrive.in" in link:
        try:
            msg = sendMessage(f"Processing: <code>{link}</code>", context.bot, update)
            link = AppDrive().appdrive_dl(link).get('gdrive_link')
            deleteMessage(context.bot, msg)
        except DirectDownloadLinkException as e:
            deleteMessage(context.bot, msg)
            return sendMessage(str(e), context.bot, update)
    ...
    if is_gdrive_link(link):
        ...
        if STOP_DUPLICATE:
            ...
            if smsg:
                msg3 = "Your link has been cloned or it was already available in Drive.\nClick the below button to get your links 👇"
        ...
        sendMessage('Send GDrive or GDToT or AppDrive or HubDrive or Sharer link along with command or by replying to the link by command', context.bot, update)
